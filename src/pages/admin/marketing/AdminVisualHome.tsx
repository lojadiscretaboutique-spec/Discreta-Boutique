...
export function AdminVisualHome() {
  ...
  const [regeneratingOffers, setRegeneratingOffers] = useState(false);

  const handleRegenerateOffersIA = async () => {
    setRegeneratingOffers(true);
    try {
      // Filtrar produtos com promoção ou desconto ativo
      const targetProducts = productsList.filter(p => p.promoPrice && p.promoPrice < p.price);
      // Se não houver produtos com desconto explícito, usar a lista geral de produtos
      const listToRank = targetProducts.length > 0 ? targetProducts : productsList;
      
      await openaiOfferSorting.rankOffers(listToRank as any[], true);
      
      toast('Vitrine premium gerada e ranqueada com IA com sucesso!', 'success');
      
      // Regenerate the general home cache as well so the changes reflect on the frontend
      await homeCacheService.regenerateHomeCache();
    } catch (err: any) {
      console.error(err);
      toast('Erro ao gerar vitrine premium com IA', 'error');
    } finally {
      setRegeneratingOffers(false);
    }
  };

  const handleRegenerateCache = async (isAutogenerated = false) => {
    setRegeneratingCache(true);
    try {
      const res = await homeCacheService.regenerateHomeCache();
      const bannersCount = res.banners?.length || 0;
      const categoriesCount = res.categories?.length || 0;
      const productsCount = res.visibleProducts?.length || 0;
      const activeSections = Object.keys(res.sections || {}).filter(k => Array.isArray((res.sections as any)[k]) && (res.sections as any)[k].length > 0).length;

      toast(
        `${isAutogenerated ? 'Cache da Home gerado automaticamente!' : 'Cache da Home regenerado com sucesso!'}
• Banners: ${bannersCount}
• Categorias: ${categoriesCount}
• Produtos: ${productsCount}
• Seções: ${activeSections}`,
        'success'
      );
    } catch (e) {
      console.error(e);
      toast('Erro ao gerar cache da Home', 'error');
    } finally {
  ...
  useEffect(() => {
    const checkCacheOnLoad = async () => {
      try {
        const cache = await homeCacheService.getHomeCache();
        if (!cache) {
          console.log("⚡ [Cache] Public home cache does not exist. Generating automatically...");
          await handleRegenerateCache(true);
        }
      } catch (err) {
        console.warn("⚠️ Error checking/auto-generating home cache on load:", err);
      }
    };
    checkCacheOnLoad();
  }, []);
    ...
    <div className="flex-1 flex flex-col bg-[#050505] text-zinc-100 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-zinc-900 pb-6 mb-8 gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button 
            disabled={regeneratingOffers}
            onClick={handleRegenerateOffersIA}
            className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 hover:border-purple-600/30 text-white rounded-xl px-4 py-2.5 font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50"
          >
            <Sparkles size={16} className={`text-purple-400 ${regeneratingOffers ? 'animate-spin' : ''}`} /> {regeneratingOffers ? 'Gerando vitrine...' : 'Regenerar vitrine premium com IA'}
          </button>

          <button 
            disabled={regeneratingCache}
            onClick={handleRegenerateCache}
            className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 hover:border-red-600/30 text-white rounded-xl px-4 py-2.5 font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50"
          >
            <RefreshCw size={16} className={`text-zinc-400 ${regeneratingCache ? 'animate-spin' : ''}`} /> {regeneratingCache ? 'Regenerando...' : 'Regenerar cache da Home'}
          </button>
